...
function deviner(){
    ...
    console.log(result); //N'affiche pas le console.log ?? 
    let nombre = Math.floor(prompt("Entrez un nombre entre 1 et 100 :"));
    while(nombre != result){
        if(nombre < result){
            nombre = Math.floor(prompt("Trop petit"));
        }
        if(nombre > result){
            nombre = Math.floor(prompt("Trop grand"));
        }
        if(nombre == result){
            nombre = alert("GG t'as gagné !");
        }
    }
}
deviner();
...
calendrier(Math.floor(prompt("Entrez un mois (1 à 12) :")));